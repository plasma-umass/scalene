...
// FIXME use the non-__APPLE__ code on __APPLE__ as well to reduce unnecessary
// variants?
class SysMallocHeap {
 public:
...
/**
 * Provides access to the original system heap, even if the standard
 * malloc/free/... have been redirected by way of LD_PRELOAD, MacOS interpose,
 * etc.
 */
class SysMallocHeap {
 ...
 public:
  ...
  SysMallocHeap()
      : _malloc((decltype(_malloc))dlsym(RTLD_NEXT, "malloc")),
        ...
        _malloc_usable_size((decltype(_malloc_usable_size))dlsym(
            RTLD_NEXT, "malloc_usable_size")) {}

  inline void* malloc(size_t sz) { return (*_malloc)(sz); }

  inline void* memalign(size_t alignment, size_t sz) {
    return (*_memalign)(alignment, sz);
  }

  inline void free(void* ptr) { (*_free)(ptr); }

  inline size_t getSize(void* ptr) { return (*_malloc_usable_size)(ptr); }
};
#endif
