...
class Scalene():
    ...
    per_line_footprint_samples    = defaultdict(lambda: defaultdict(lambda: adaptive.adaptive(18)))
    ...
    def output_profiles():
        ...
        with Scalene.file_or_stdout(Scalene.output_file) as out:
            ...
            for fname in sorted(all_instrumented_files):
                ...
                print("       |%9s |%9s | %s %s " % ('CPU %', 'CPU %', ' Net  |' if did_sample_memory else '', 'Memory                    |' if did_sample_memory else ''), file=out)
                print("  Line |%9s |%9s | %s %s [%s]" % ('(Python)', '(native)', ' (MB) |' if did_sample_memory else '', 'usage                     |' if did_sample_memory else '', fname), file=out)
                print("-" * 80, file=out)
                ...
                with open(fname, 'r') as source_file:
                    for line_no, line in enumerate(source_file, 1):
                        ...
                        if did_sample_memory:
                            ...
                            print("%6d |%9s |%9s | %5s | %-18s %-6s | %s" %
                                  (line_no, n_cpu_percent_python_str, n_cpu_percent_c_str, n_growth_mb_str, spark_string, n_usage_fraction_str, line), file=out)
                        else:
                            print("%6d |%9s |%9s | %s" %
